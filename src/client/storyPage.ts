import { DoubleSide, Group, Material, Mesh, MeshBasicMaterial, 
    PlaneGeometry, Scene, TextureLoader, Vector3 } from "three"
...
class StoryPage {
    ...
    constructor(
        ...
        this.storyText = this.createStoryText()
        this.frameGroup.add(this.progressText)
        this.frameGroup.add(this.storyText)
    ...
    createStoryText() {
        const newTitleText = new Mesh(
            new TextGeometry(
                this.storyInput[this.photoIndex - 1],
                {
                    font: getOrnateFont(),
                    size: this.frameSize/24,
                    height: 0.1,
    ...
    changeStoryText() {
        this.frameGroup.remove(this.storyText)
        this.scene.remove(this.storyText);
        (this.storyText.material as Material).dispose()
        this.storyText.geometry.dispose()
